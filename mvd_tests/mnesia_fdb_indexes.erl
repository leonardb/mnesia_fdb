...
run() ->
    mnesia:stop(),
    ok = mnesia_fdb_tlib:start_mnesia(reset),
    ...
    fail(test, [1, fdb, l1]), % doesn't support bag
    test(3, fdb, l1),
    add_del_indexes(),
    {atomic,ok} = mnesia_schema:add_index_plugin(
                    {pfx},mnesia_fdb, ix_prefixes),
    ...
    test_index_plugin(pl2, fdb, ordered),
