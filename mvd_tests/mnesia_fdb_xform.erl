...
%% extension machinery
%%
%% Usage: mnesia_ext_fdb_test:recompile(Extension).
%% Usage: mnesia_ext_fdb_test:recompile().
...
create_table(Parameters) when is_list(Parameters) ->
    case lists:keymember(fdb_copies, 1, Parameters) of
        true ->
            %% case lists:member({type, bag}, Parameters) of
            %%     true ->
            %%         ct:comment("ERROR: Contains fdb table with bag"),
            %%         {aborted, {fdb_does_not_support_bag, Parameters}};
            %%     false ->
            ct:comment("INFO: Contains fdb table"),
