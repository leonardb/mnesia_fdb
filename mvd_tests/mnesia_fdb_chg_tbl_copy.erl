...
-module(mnesia_fdb_chg_tbl_copy).
...
run() ->
    run([fdb,disc_copies,fdb,ram_copies,disc_only_copies,fdb]).
    %% run([fdb,disc_only_copies]).
    %% run([fdb,ram_copies]).
...
copies() ->
    [fdb,ram_copies,disc_copies,disc_only_copies].
...
start_mnesia() -> mnesia_fdb_tlib:start_mnesia(reset).
